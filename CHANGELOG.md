...
- `SerialPort` 将开启与关闭功能与脚本的enable状态相连接，当脚本被关闭时会自动关闭串口连接，启用时尝试连接串口，以便于手动控制串口连接状态

## [5.3.3] - 2026-3-9
### 更改
- `SerialPort` 修复了串口连接的关闭日志输出错误的BUG，现在会根据串口是否成功打开，输出串口是否打开的日志提示
- `SerialPort` 修复了在接收数据时，当有效数据长度设为<=0时未正确处理长度导致的数据错误，同时修改了数据裁剪逻辑：当设为非负值时，将末尾裁剪掉对应绝对值长度的数据要求（当设为0时，截取从开始位到数据包结尾）
- `SerialPort` 提供了 `showFullDebugLog` 和 `showFullEventLog` 用于在运行时控制是否输出完整的调试日志和事件日志，便于调试
