...
namespace DreemurrStudio.SerialPortSystem
{
    public class SerialPortManager : MonoBehaviour
    {
        ...
        /// 串口对象实例列表
        /// </summary>
        private static List<SerialPortManager> _instances = new List<SerialPortManager>();

        /// <summary>
        /// 建议在全局仅一个串口时使用，获取第一个同时也是唯一一个串口实例
        /// </summary>
        /// <returns></returns>
        public static SerialPortManager GetInstance() => _instances[0];
        /// <summary>
        /// 根据对象名获取串口实例
        /// </summary>
        /// <param name="objectName">串口对象名</param>
        /// <returns></returns>
        public static SerialPortManager GetInstance(string objectName)
        {
        ...
        /// <returns></returns>
        public static SerialPortManager GetInstance(int id)
        {
            if (_instances.Count == 0) throw new Exception("串口实例尚未完成初始化！");
        ...
        [TitleGroup("端口配置")]
        [SerializeField]
        [Tooltip("接收数据缓冲池（字节数组）的长度，根据实际情况调整，建议在64~1024之间且为2的幂")]
        ...
        private void OnEnable()
        {
            ...
            {
                Debug.LogError($"串口{PortName}打开失败: {ex.Message}");
            }
            Debug.Log($"{PortName}串口已成功开启");
            _instances.Add(this);
            //初始化数据缓存池
            receiveBytesBuffer = new List<byte>(bufferLength);
            //开启数据读取协程
            readCoroutine = StartCoroutine(ReadSerialPortCoroutine());
        }
        ...
        private void OnDisable()
        {
            ...
            {
                serialPort.Close();
                Debug.Log($"{PortName}串口已成功关闭");
            }
            catch (Exception ex)
            {
                Debug.LogError($"串口{PortName}关闭失败: {ex.Message}");
            }
        }
        ...
        #region 数据接收
        /// <summary>
        /// 串口数据读取协程，根据配置的通信方式进行数据读取与处理，并通过事件或委托转发数据
        /// </summary>
        /// <returns></returns>
        private IEnumerator ReadSerialPortCoroutine()
        {
            while (serialPort != null && serialPort.IsOpen)
            {
                try
                {
                    if (serialPort.BytesToRead > 0)
                    {
                        ...
                        {
                            case DataFormat.hex:
                                var bd = new byte[bufferLength];
                                var readLength = serialPort.Read(bd, 0, bufferLength);
                                if (readLength > 0) OnReceive(bd, readLength);
                                break;
                            case DataFormat.ascii:
                                var sd = new char[bufferLength];
                                readLength = serialPort.Read(sd, 0, bufferLength);
                                var s = new string(sd);
                                if (readLength > 0) OnReceive(s, s.Length);
                                break;
                        }
                    }
                ...
                {
                    Debug.LogError($"串口{PortName}读取数据失败: {ex.Message}");
                }
                yield return null;
            }
        }
